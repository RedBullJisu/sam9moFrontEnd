<template>
  <div>
    ...
    <div v-if="selectedMarket === 'kosdaq'">
...
export default {
  ...
  setup() {
    ...
    const fetchData = async (timeline, series) => {
      const url = `http://221.156.60.18:9199/kosdaq/kosdaq_${timeline}`;
      ...
      series['kosdaq'] = [{name: 'kosdaq', data: processedData}];
    };
    const fetchVolumeData = async (timeline) => {
      ...
      if (!state.volumeChartseries['kosdaq']) {
        state.volumeChartseries['kosdaq'] = [];
      }

      state.volumeChartseries['kosdaq'] = [{name: 'kosdaq', data: processedData}];
    };

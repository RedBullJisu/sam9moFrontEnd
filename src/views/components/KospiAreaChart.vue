...
export default {
  ...
  setup() {
    ...
    const fetchData = async (timeline, series) => {
      const url = `http://221.156.60.18:9199/kospi/kospi_${timeline}`;
      ...
      series['kospi'] = [{name: 'kospi', data: processedData}];
    };
    const fetchVolumeData = async (timeline) => {
      ...
      if (!state.volumeChartseries['kospi']) {
        state.volumeChartseries['kospi'] = [];
      }

      state.volumeChartseries['kospi'] = [{name: 'kospi', data: processedData}];
    };
